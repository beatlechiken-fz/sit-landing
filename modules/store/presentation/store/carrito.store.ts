...
export const useCarritoStore = create<CarritoState>()(
  persist(
    (set, get) => ({
      ...
      subtotalCarrito: () =>
        get().lineas.reduce((acc, l) => acc + l.subtotal, 0),
      descuentoLineas: () =>
        get().lineas.reduce((acc, l) => acc + l.descuento, 0),
      ...
      descuentoGlobal: () => {
        ...
        if (cuponGlobal.tipo === "porcentaje") {
          return (
            Math.round(((totalSinGlobal * cuponGlobal.descuento) / 100) * 100) /
            100
          );
        }
